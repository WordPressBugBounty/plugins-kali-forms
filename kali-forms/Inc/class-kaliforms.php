...
class KaliForms {
	...
	public function __construct() {
		...
		register_activation_hook( KALIFORMS_PLUGIN_FILE, array( $this, 'install' ) );
		// Welcome screen init
		// new Welcome_Screen();
		// Init Kali Forms plugin during the plugins_loaded hook. It provides an order for the "additional" components
		add_action( 'plugins_loaded', array( $this, 'init_kaliforms' ) );
		add_action( 'admin_head', array( $this, 'upgrade_pro' ) );
	}
	...
	public function init_kaliforms() {
		 ...
		 */
		add_action( $this->slug . '_delete_transient_file', array( $this, 'delete_transient_file' ) );
		add_action( 'rest_api_init', array( $this, 'start_controller' ) );
		new License_Checker();
	}

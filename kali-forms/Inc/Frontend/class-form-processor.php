...
class Form_Processor {
	 ...
	 * @var array
	 */
	public $placeholdered_data = array();
	 ...
	 * @var array
	 */
	public $advanced_field_map = array();
	...
	public $spoofer = array();
	/**
	 * Class constructor
	 */
	public function __construct() {
		add_action( 'wp_ajax_kaliforms_form_process', array( $this, 'form_process' ) );
		add_action( 'wp_ajax_nopriv_kaliforms_form_process', array( $this, 'form_process' ) );

		add_action( 'wp_ajax_kaliforms_preflight', array( $this, 'preflight' ) );
		add_action( 'wp_ajax_nopriv_kaliforms_preflight', array( $this, 'preflight' ) );

		add_action( 'wp_ajax_kaliforms_form_verify_recaptcha', array( $this, 'verify_recaptcha' ) );
		add_action( 'wp_ajax_nopriv_kaliforms_form_verify_recaptcha', array( $this, 'verify_recaptcha' ) );

		add_action( 'wp_ajax_kaliforms_form_verify_turnstile', array( $this, 'verify_turnstile' ) );
		add_action( 'wp_ajax_nopriv_kaliforms_form_verify_turnstile', array( $this, 'verify_turnstile' ) );

		add_action( 'wp_ajax_kaliforms_form_upload_file', array( $this, 'upload_file' ) );
		add_action( 'wp_ajax_nopriv_kaliforms_form_upload_file', array( $this, 'upload_file' ) );

		add_action( 'wp_ajax_kaliforms_form_delete_uploaded_file', array( $this, 'delete_file' ) );
		add_action( 'wp_ajax_nopriv_kaliforms_form_delete_uploaded_file', array( $this, 'delete_file' ) );
	}
	...
	public function form_process() {
			...
			wp_die(
				wp_json_encode(
					array(
						...
						'error_bag'         => isset( $this->data['error_bag'] ) ? $this->data['error_bag'] : array(),
		...
		if ( isset( $this->data['admin_stop_execution'] ) && $this->data['admin_stop_execution'] ) {
			...
			$response['error_bag']         = isset( $this->data['error_bag'] ) ? $this->data['error_bag'] : array();
		}
		/**
		 * Return a response to the frontend
		 */
	...
	public function check_if_placeholders_changed() {
		...
		foreach ( $this->data as $key => $value ) {
			...
			// Reserved internal placeholders must never be overwritten by field POST data.
			if ( $this->is_reserved_placeholder_field( $key ) ) {
				continue;
			}
		...
		// admin_external_change / media spoofing can re-run this after the initial restore.
	...
	public function prepare_post_data( $data ) {
		...
		foreach ( $data as $k => $v ) {
			...
			if ( $this->is_reserved_placeholder_field( $k ) ) {
				continue;
			}
		...
		foreach ( $placeholders as $placeholder ) {
			if ( in_array( $placeholder, array( '{allFields}', '{formName}' ) ) ) {
				continue;
			}

			if ( strpos( $placeholder, ':' ) > -1 ) {
				if ( ! empty( $hooks ) ) {
					$curr_placeholder = str_replace( array( '{', '}' ), '', $placeholder );
	...
	/**
	 * Field names that collide with reserved internal placeholder callbacks.
	 *
	 * @return string[]
	 */
	private function get_reserved_placeholder_field_names() {
		return array( 'entryCounter', 'thisPermalink', 'submission_link' );
	}

	/**
	 * Whether a submitted field name maps to a reserved internal placeholder.
	 *
	 * @param string $key Field name from POST / field_components.
	 * @return bool
	 */
	private function is_reserved_placeholder_field( $key ) {
		return in_array( $key, $this->get_reserved_placeholder_field_names(), true );
	}
	...
	private function restore_internal_callable_placeholders() {
		...
		foreach ( array( '{entryCounter}', '{thisPermalink}', '{submission_link}' ) as $key ) {
	 ...
	 *
	 * Never call call_user_func() on values that may have been influenced by request data.
	 *
	 * @param string $placeholder_key
	 * @param array  $args
	 * @return mixed
	 */
	private function invoke_trusted_callable( $placeholder_key, array $args = array() ) {
		$defaults = ( new GeneralPlaceholders() )->general_placeholders;
	...
	protected function _run_placeholder_switch( $type, $k, $v ) {
		...
		switch ( $type ) {
			...
			case 'imageRadio':
				...
				$choice_labels = array();
				foreach ( $this->advanced_field_map[ $k ]->choices as $choice ) {
					$choice_labels[ $choice->image->id ] = array(
						'label'   => $choice->label,
	...
	protected function _save_data() {
		...
		if ( isset( $this->placeholdered_data['{entryCounter}'] ) ) {
			$this->placeholdered_data['{entryCounter}'] = $this->invoke_trusted_callable( '{entryCounter}', array( $this->post->ID ) );
		}
		if ( isset( $this->placeholdered_data['{thisPermalink}'] ) ) {
			$this->placeholdered_data['{thisPermalink}'] = $this->invoke_trusted_callable( '{thisPermalink}' );
		}

		if ( isset( $this->placeholdered_data['{submission_link}'] ) && $this->saved ) {
			$this->placeholdered_data['{submission_link}'] = $this->invoke_trusted_callable(
				'{submission_link}',
				array(
	...
	private function filter_submission_data_for_storage( array $data ) {
		...
		foreach ( $data as $k => $v ) {
			...
			if ( in_array( $k, array( 'formId', 'nonce', 'grecaptcha', 'g-recaptcha-response', 'kf_submitted_user_id' ), true ) ) {
				continue;
			}
			...
			if ( isset( $_POST['extraArgs'] ) && ! empty( $_POST['extraArgs'] ) && count( $_POST['extraArgs'] ) ) {
				...
				foreach ( $data as $uploadField ) {
					...
					foreach ( $uniqueIds as $uId ) {
						...
						wp_clear_scheduled_hook( $this->slug . '_delete_transient_file', array( absint( $actualId ) ) );
